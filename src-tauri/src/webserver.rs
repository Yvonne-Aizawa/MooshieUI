    ...
    match command {
        ...
        "get_lora_civitai_info" | "get_checkpoint_civitai_info" => {
            ...
            if let Some(data) = civitai_data {
                ...
                result["civitai_source"] = serde_json::json!("civitai");
            }

            // Fallback to CivArchive if CivitAI lookup failed
            if result.get("civitai_model_id").is_none() || result.get("civitai_name").is_none() {
                match commands::api::scrape_civarchive_page(&state.http_client, &sha256).await {
                    Ok(models) => {
                        // Use the first model (most relevant)
                        if let Some(model) = models.first() {
                            if result["civitai_name"].is_null() {
                                result["civitai_name"] =
                                    serde_json::json!(model.get("name").and_then(|v| v.as_str()));
                            }
                            if result["civitai_description"].is_null() {
                                result["civitai_description"] = serde_json::json!(model
                                    .get("description")
                                    .and_then(|v| v.as_str()));
                            }
                            if result["civitai_creator"].is_null() {
                                result["civitai_creator"] = serde_json::json!(model
                                    .get("username")
                            ...
                            if let Some(version) = model.get("version") {
                                if result["civitai_base_model"].is_null() {
                                    result["civitai_base_model"] = serde_json::json!(version
                                        .get("base_model")
                                        .and_then(|v| v.as_str()));
                                }
                                ...
                                // Stats
                                if result["civitai_download_count"].is_null() {
                                    result["civitai_download_count"] = serde_json::json!(version
                                        .get("download_count")
                                        .and_then(|v| v.as_u64()));
                                }
                                if result["civitai_thumbs_up_count"].is_null() {
                                    if let Some(rating) =
                                        version.get("rating").and_then(|v| v.as_f64())
                                    {
                                        result["civitai_thumbs_up_count"] =
                                            serde_json::json!((rating * 20.0) as u64);
                                    }
                                }

                                // Images
                                if result["civitai_images"].is_null() {
                                    if let Some(images) =
                                        version.get("images").and_then(|v| v.as_array())
                                    {
                                        let imgs: Vec<serde_json::Value> = images.iter().filter_map(|img| {
                                            img.get("url").and_then(|u| u.as_str()).map(|url| {
                                                serde_json::json!({
                                        ...
                                        if command == "get_checkpoint_civitai_info"
                                            && result.get("thumbnail_url").is_none()
                                        {
                                            result["thumbnail_url"] = serde_json::json!(imgs
                                                .first()
